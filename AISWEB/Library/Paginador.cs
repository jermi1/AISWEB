    ...
    public class Paginador<T>
    {
            ...
            String action, String host)
        {
            ...
            if (pagi_actual != 1)
            {
                ...
                pagi_navegacion += "<a class='btn btn-info' href='" + host + "/" + controller + "/"
                    + action + "?id=" + pagi_url + "&registros=" + pagi_cuantos + "&area=" + area + "'>"
                    + pagi_nav_primera + "</a>"; // botones de navegacion

                // Si no estamos en la página 1. Ponemos el enlace "anterior" 
                pagi_url = pagi_actual - 1; //será el número de página al que enlazamos 
                pagi_navegacion += "<a class='btn btn-info' href='" + host + "/" + controller + "/" + action
            ...
            double valor2 = (pagi_nav_num_enlaces / 2); //calcular el inicio y el final
            ...
            for (int pagi_i = pagi_nav_desde; pagi_i <= pagi_nav_hasta; pagi_i++)
            {
                ...
                if (pagi_i == pagi_actual)
                {
                    ...
                    pagi_navegacion += "<span class='btn btn-link' disabled='disabled'>" + pagi_i + "</span>";
                }
                else
                {
                    // Si es cualquier otro. Se escribe el enlace a dicho número de página. 
                    pagi_navegacion += "<a class='btn btn-link' href='" + host + "/" + controller + "/" +
